...
| `___3079ch.c` | when an event is signed up for, one AI seat becomes the adversary (name, car 6, full equipment, `points = best of the others + 20`), and he joins the field as a normal candidate for the first opponent slot - the "picked" flag then keeps him to **one race per event**, like everybody else.  The seeding happens here and not when the roster is built, because the difficulty is chosen in the licence screen, i.e. *after* the roster exists (his car is the marker, so it survives the roster being sorted and stays a one-off) |
