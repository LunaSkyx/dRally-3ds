...
| `___38184h.c` | the standings draw each racer's car picture out of `carres.bpk` (`0x5140` bytes per car, the file being `0x1e780` = six cars).  Car 6 has no picture of its own, so `DR3_ADVERSARY_CAR_PIC` (5 - the Deliverator, the SPECIAL being its black twin) is used instead of reading past the end of that table; the face index is clamped to `face01..face20` as well |
| `___3079ch.c` (his picture) | his **driver picture** is turned black and white, once: the faces are palette-indexed pictures (MENU.BPA `face01..face20`, 0x40x0x40), so every pixel index of his face is replaced by the index of the nearest neutral grey of the same brightness in the palette that is active at that moment - those being the greys the front end itself draws its frames and dialogs with.  Every screen that shows him picks it up (signup roster, licence screen, standings), because they all read the same picture |
