...
| `___3079ch.c` | when an event is signed up for, one AI seat becomes the adversary (name, car 6, full equipment, `points = best of the others + 20` - the seeding happens here and not when the roster is built, because the difficulty is chosen in the licence screen, i.e. *after* the roster exists).  He is then a **candidate like any other racer for the tier the player is signing up for** (`D(___185a50h)`; if that tier is full he takes one of the other two), so he is in one of the three races of every event.  "One race per event" is enforced by the event's field itself (`___1a0ef8h`), not by the engine's "picked" flag, which is only cleared when the signup screen is entered.  His **seat is not fixed** - the roster is sorted by points after every race (`___30a84h`) - so he is always found through his car, and the car is never touched by a race result |
