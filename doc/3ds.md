...
| `___3079ch.c` | when an event is signed up for, one AI seat becomes the adversary (name, car 6, full equipment, and `points` set to **third place**: just below the second-best score, so he has to work his way up).  The seeding happens here and not when the roster is built, because the difficulty is chosen in the licence screen, i.e. *after* the roster exists.  He then joins the field as a **candidate like any other racer** (the first free slot of the event is his, and "one race per event" is enforced by the event's field array `___1a0ef8h` itself, not by the engine's "picked" flag, which is only reset when the signup screen is entered).  His **seat is not fixed** - the roster is sorted by points after every race (`___30a84h`) - so he is always found through his car, and no race result ever touches a racer's car |
| `___3266ch.c` | the signup: once the field is filled and the player confirms a tier, `dr3_adversary_join(tier)` makes sure the adversary is one of the opponents of **that** race - if the picker missed him, or if the player switched tiers while the lists were filling up.  If he was in another tier's grid, the racer he displaces takes that old slot, so no racer ever appears twice in one grid (and the player's own slot is left alone) |
